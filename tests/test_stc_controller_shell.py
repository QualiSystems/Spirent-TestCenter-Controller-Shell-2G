...
target = 'StcControllerShell2G'

controller = 'localhost'
port = '8888'

ports = ['offline-debug-152/Module1/PG1/Port1', 'offline-debug-152/Module1/PG1/Port2']
attributes = [AttributeNameValue('StcControllerShell2G.Address', controller),
              AttributeNameValue('StcControllerShell2G.Controller TCP Port', port)]
...
class TestStcControllerShell(object):
    ...
    def setup(self):
        ...
        self.context = create_command_context(self.session, ports, target, attributes)
    ...
    def test_session_id(self):
        session_id = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
                                                 'Service', 'get_session_id')
        print('session_id = {}'.format(session_id.Output))
        project = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
        ...
        project_childs = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
                                                     'Service', 'get_children',
                                                     [InputNameValue('obj_ref', project_obj)])
        print('Project-Children = {}'.format(project_childs.Output))

        options = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
                                              'Service', 'get_children',
                                              [InputNameValue('obj_ref', 'system1'),
        ...
        options_attrs = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
                                                    'Service', 'get_attributes',
                                                    [InputNameValue('obj_ref', options_ref)])
        print('AutomationOptions-Attributes = {}'.format(options_attrs.Output))

        self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
    ...
    def test_set_device_params(self):
        ...
        devices = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
                                              'Service', 'get_children',
                                              [InputNameValue('obj_ref', project_obj),
        ...
        device_1_attrs = self.session.ExecuteCommand(self.context.reservation.reservation_id, target,
                                                     'Service', 'get_attributes',
    ...
    def test_run_traffic(self):
        self._load_config(path.join(path.dirname(__file__), 'test_config.tcc'))
        self.session.ExecuteCommand(self.context.reservation.reservation_id, target, 'Service',
                                    'send_arp')
        self.session.ExecuteCommand(self.context.reservation.reservation_id, target, 'Service',
                                    'start_devices')
        self.session.ExecuteCommand(self.context.reservation.reservation_id, target, 'Service',
