...
def driver(test_helpers: TestHelpers, server: list) -> StcControllerShell2GDriver:
    ...
    init_context = test_helpers.service_init_command_context(
        STC_CONTROLLER_MODEL, attributes
    )
...
@pytest.fixture()
def context(
    session: CloudShellAPISession, test_helpers: TestHelpers, server: list
    ...
    session.AddServiceToReservation(
        test_helpers.reservation_id, STC_CONTROLLER_MODEL, ALIAS, attributes
    )
...
class TestStcControllerDriver:
    ...
    def test_get_set(
        self, driver: StcControllerShell2GDriver, context: ResourceCommandContext
    ) -> None:
    ...
    def test_negative(
        ...
        set_family_attribute(
            context, reservation_ports[0].Name, "Logical Name", "Port 1"
        )
        set_family_attribute(context, reservation_ports[1].Name, "Logical Name", "")
        with pytest.raises(TgnError):
            driver.load_config(
                context, Path(__file__).parent.joinpath("test_config.tcc")
            )
        set_family_attribute(
            context, reservation_ports[1].Name, "Logical Name", "Port 1"
        )
        with pytest.raises(TgnError):
            driver.load_config(
                context, Path(__file__).parent.joinpath("test_config.tcc")
        ...
        set_family_attribute(
            context, reservation_ports[1].Name, "Logical Name", "Port x"
        )
        with pytest.raises(TgnError):
            driver.load_config(
                context, Path(__file__).parent.joinpath("test_config.tcc")
        ...
        set_family_attribute(
            context, reservation_ports[1].Name, "Logical Name", "Port 2"
        )

    @pytest.mark.usefixtures("skip_if_offline")
    def test_run_traffic(
        self, driver: StcControllerShell2GDriver, context: ResourceCommandContext
    ) -> None:
        config_file = Path(__file__).parent.joinpath("test_config.tcc")
        self._load_config(driver, context, config_file)
...
class TestStcControllerShell:
    ...
    def test_session_id(
        self, session: CloudShellAPISession, context: ResourceCommandContext
    ) -> None:
        session_id = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_session_id"
        )
        ...
        project = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_children", cmd_inputs
        )
        ...
        project_children = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_children", cmd_inputs
        )
        ...
        options = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_children", cmd_inputs
        )
        options_ref = json.loads(options.Output)[0]
        cmd_inputs = [InputNameValue("obj_ref", options_ref)]
        old_options_attrs = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_attributes", cmd_inputs
        )
        ...
        session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "set_attribute", cmd_inputs
        )
        cmd_inputs = [InputNameValue("obj_ref", options_ref)]
        new_options_attrs = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_attributes", cmd_inputs
        )
        assert (
            json.loads(old_options_attrs.Output)["LogLevel"]
            != json.loads(new_options_attrs.Output)["LogLevel"]
        )
        ...
        output = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "perform_command", cmd_inputs
        )
    ...
    def test_load_config(
        ...
        self._load_config(
            session, context, Path(__file__).parent.joinpath("test_config.tcc")
        )
    ...
    def test_run_traffic(
        ...
        session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "send_arp"
        )
        session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "start_protocols"
        )
        cmd_inputs = [InputNameValue("blocking", "True")]
        session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "start_traffic", cmd_inputs
        )
        time.sleep(2)
        cmd_inputs = [
        ...
        stats = session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "get_statistics", cmd_inputs
        )
    ...
    def test_run_sequencer(
        ...
        self._load_config(
            session, context, Path(__file__).parent.joinpath("test_sequencer.tcc")
        )
        cmd_inputs = [InputNameValue("command", "Start")]
        session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "run_quick_test", cmd_inputs
        )
        ...
        assert (
            int(json.loads(stats.Output)["Port 1"]["GeneratorIpv4FrameCount"]) == 8000
        )
    ...
    def _load_config(
        ...
        session.ExecuteCommand(
            get_reservation_id(context), ALIAS, "Service", "load_config", cmd_inputs
        )
